 ...
 * Forces Google Sheets sync on every load and save.
 * Bookmarks are stored in a Google Sheet (via a Google Apps Script
 * Web App backend) and loaded fresh on every page visit from any device.
 */
...
const cfg = {
  ...
  save({ sheetUrl, sheetKey }) {
    ...
    if (sheetKey && !sheetKey.includes('•')) {
      localStorage.setItem('bh-sheet-key', sheetKey);
    }
  },

  isConfigured() {
    return !!(this.sheetUrl && this.sheetKey);
  },
};
...
════════════════════════════════════════════════════════════ */
const sheetsApi = {
  /* READ bookmarks + categories from the Sheet */
  async load() {
    if (!cfg.isConfigured()) {
    ...
    const url = `${cfg.sheetUrl}?key=${encodeURIComponent(cfg.sheetKey)}&t=${Date.now()}`;
  ...
  /* WRITE bookmarks + categories to the Sheet (full overwrite) */
  async save() {
    if (!cfg.isConfigured()) {
    ...
    // Content-Type text/plain avoids a CORS preflight (Apps Script has no
    // doOptions handler); the body itself is still valid JSON, parsed
    // server-side.
    ...
    if (!res.ok) {
      throw new Error(`Google Sheets write error ${res.status}`);
    }
  ...
  async testConnection() {
    ...
    if (!data) throw new Error('No response from the Web App URL.');
    return `Connected — ${data.bookmarks.length} bookmark(s), ${data.categories.length} categorie(s) found.`;
  },
};
...
/* ════════════════════════════════════════════════════════════
   PERSIST — always tries Google Sheets first
════════════════════════════════════════════════════════════ */
async function persistData(successMsg = 'Saved.') {
  local.save(); // always save locally as backup

  if (!cfg.isConfigured()) {
    toast('Saved locally only. Configure Google Sheets Settings to sync across devices.', 'info');
    return;
  }
  ...
  try {
    ...
    toast('✓ ' + successMsg + ' Synced to Google Sheets — visible on all devices!', 'success');
  } catch (err) {
    console.error('Google Sheets sync failed:', err);
    toast('⚠ Saved locally but Google Sheets sync FAILED: ' + err.message, 'error');
  } finally {
...
function openSettingsModal() {
  document.getElementById('sheet-url').value = cfg.sheetUrl;
  document.getElementById('sheet-key').value = cfg.sheetKey ? '••••••••••••••••' : '';
...
function saveSettings() {
  ...
  const rawKey   = document.getElementById('sheet-key').value.trim();
  const sheetKey = rawKey.includes('•') ? cfg.sheetKey : rawKey;

  if (!sheetUrl || !sheetKey) {
    toast('Please fill in both the Web App URL and the secret key.','error');
    return;
  }

  cfg.save({ sheetUrl, sheetKey });
  closeAllModals();
  toast('Settings saved! Reloading data from Google Sheets…','success');

  // Immediately reload from Google Sheets with new settings
...
async function testConnection() {
  ...
  const rawKey   = document.getElementById('sheet-key').value.trim();
  const sheetKey = rawKey.includes('•') ? cfg.sheetKey : rawKey;
  if (!sheetUrl || !sheetKey) { statusEl.textContent='Enter the Web App URL and key first.'; statusEl.className='connection-status err'; return; }

  const prevUrl = cfg.sheetUrl;
  const prevKey = cfg.sheetKey;
  localStorage.setItem('bh-sheet-url', sheetUrl);
  localStorage.setItem('bh-sheet-key', sheetKey);
  statusEl.textContent='Testing…'; statusEl.className='connection-status';

  try {
    const msg = await sheetsApi.testConnection();
    statusEl.textContent=`✓ ${msg}`;
    statusEl.className='connection-status ok';
  } catch(err) {
    statusEl.textContent='✗ '+err.message;
    statusEl.className='connection-status err';
    localStorage.setItem('bh-sheet-url', prevUrl);
    localStorage.setItem('bh-sheet-key', prevKey);
  }
}
...
/* ════════════════════════════════════════════════════════════
   LOAD FROM GOOGLE SHEETS (called on startup and after settings save)
════════════════════════════════════════════════════════════ */
async function loadFromSheets() {
  ...
  try {
    const data = await sheetsApi.load();
    if (data && (data.bookmarks.length > 0 || data.categories.length > 0)) {
      state.bookmarks  = data.bookmarks  || [];
      state.categories = data.categories || state.categories;
      local.save(); // update local copy
      toast('✓ Loaded from Google Sheets successfully!','success');
    } else {
      // Sheet is empty — upload local data if any
      ...
      if (localData && localData.bookmarks && localData.bookmarks.length > 0) {
        ...
        toast('Sheet is empty. Uploading your local bookmarks now…','info');
        await sheetsApi.save();
        toast('✓ Local bookmarks uploaded to Google Sheets!','success');
      } else {
        toast('Google Sheets connected! Add your first bookmark.','info');
      }
    }
  } catch(err) {
    console.error('Google Sheets load error:', err);
    toast('Google Sheets load failed: ' + err.message + ' — showing local data.','error');
    const localData = local.load();
    if (localData) {
      state.bookmarks  = localData.bookmarks  || [];
      state.categories = localData.categories || state.categories;
...
async function init() {
  ...
  if (cfg.isConfigured()) {
    // Always load fresh from Google Sheets on every page load
    ...
    render();
    toast('Configure Google Sheets Settings (⚙) to sync across devices.','info');
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('toggle-token-vis').addEventListener('click', ()=>{
    const inp = document.getElementById('sheet-key');
